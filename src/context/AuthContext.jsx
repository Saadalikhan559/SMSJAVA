...
export const AuthProvider = ({ children }) => {
  ...
  const [teacherID, setTeacherID] = useState(() => localStorage.getItem("teacher_id") || "");
  const [guardianID, setGuardianID] = useState(() => localStorage.getItem("guardian_id") || "");
  ...
  const [yearLevelID, setYearLevelID] = useState(() => localStorage.getItem("year_level_id") || "");
  const [stuyearLevelID, setstuYearLevelID] = useState(() => localStorage.getItem("stu_year_level_id") || "");
  const [stuyearLevelName, setstuYearLevelName] = useState(() => localStorage.getItem("stu_year_level_name") || "");
  const [loading, setLoading] = useState(true);
  ...
  const LoginUser = async (userDetails) => {
    try {
      ...
      // Save tokens
      const tokens = { access: data.accessToken, refresh: data.refreshToken };
      setAuthTokens(tokens);
      authTokensRef.current = tokens;
      localStorage.setItem("authTokens", JSON.stringify(tokens));
      ...
      // Save profile (fallback if null)
      const normalizedProfile = data.userProfile
        ? normalizeProfileUrl(data.userProfile)
