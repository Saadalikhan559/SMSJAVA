...
export const PaySalaryExpense = () => {
  ...
  // 🔹 Fetch employee data (JAVA TOKEN via axiosInstance)
  useEffect(() => {
    const fetchSingleSalaryData = async () => {
      try {
        setPageLoading(true);
...
//         payment_method: data.payment_method.toUpperCase(), // CASH / CHEQUE / ONLINE
  ...
  payload.cheque_number = data.cheque_number;
      ...
      if (data.payment_method === "ONLINE") {
        const orderResponse = await axiosInstance.post(
          `${JAVA_BASE_URL}/employee-salary/create-bulk`,
          payload
        );
        ...
        if (
          ...
          setAlertMessage(orderResponse.data?.message || "Salary created");
          setShowAlert(true);
          return;
        }
