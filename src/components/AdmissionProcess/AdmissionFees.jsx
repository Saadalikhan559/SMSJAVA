...
//         `${JAVA_BASE_URL}/student-fees/feePreview?student_year_id=${studentId}`,
...
//   availableFees.forEach((monthData) => {
//   const monthId = monthData.month_id || monthMap[monthData.month.toUpperCase()];
//      console.log("Month:", monthData.month, "Month ID:", monthId); 
...
//         `${JAVA_BASE_URL}/student-fees/initiatePayment`,
//         payload
//       );
...
//             `${JAVA_BASE_URL}/student-fees/confirmPayment`,
//             verifyPayload
//           );
...
//         `${JAVA_BASE_URL}/student-fees/submitFee`,
//         payload
//       );
...
//   console.log("Payment Payload:", payload); // Debug ke liye
...
const onSubmit = async (data) => {
  ...
  availableFees.forEach((monthData) => {
    const monthId =
      monthData.month_id || monthMap[monthData.month?.toUpperCase()];
    console.log("Month:", monthData.month, "Month ID:", monthId);
  ...
  try {
    if (paymentMode === "ONLINE") {
      ...
      const rzp = new window.Razorpay(options);
      rzp.open();
      // ============ ONLINE PAYMENT END - UNCHANGED ============
    } else {
      // ============ CASH/CHEQUE PAYMENT - FIXED ============
      console.log("========== OFFLINE PAYMENT DEBUG START ==========");
      ...
      console.log("Response Data:", JSON.stringify(submitRes.data, null, 2));
      ...
      if (submitRes.data) {
        ...
        const monthsArray =
          submitRes.data.data?.map((fee) => monthNames[fee.month]) || [];
        const uniqueMonths = [...new Set(monthsArray)].join(", ");

        // Enrich response with all required fields for receipt
        const enrichedPaymentData = {
          // From API response
          payment_mode: submitRes.data.payment_mode || paymentMode,
          data: submitRes.data.data || [],
          message:
            submitRes.data.message || "Payment recorded successfully",

          // Generated/Enriched fields
          receipt_number: `RCP-${Date.now()}`,
          payment_date: new Date().toISOString(),
          months: uniqueMonths || "N/A",

          // Student info from selected student
          student: {
            name: selectedStudent?.student_name || "N/A",
            scholar_number: selectedStudent?.scholar_number || "N/A",
            class_name: selectedStudent?.year_level_name || "N/A",
          },

          // Payment details
          total_amount: submitRes.data.total_amount_paid || paidAmount,
          paid_amount: submitRes.data.total_amount_paid || paidAmount,
          total_amount_paid: submitRes.data.total_amount_paid || paidAmount,

          // Additional fields
          received_by: "Office Staff",
          remarks: data.remarks || "N/A",
          is_cheque_cleared: paymentMode === "CHEQUE" ? false : null,
          late_fee: 0,
        };

        console.log("Enriched Payment Data:", enrichedPaymentData);
        setPaymentStatus(enrichedPaymentData);
        setShowPaymentDialog1(true);
      } else {
        alert("Payment recorded but receipt data not received.");
      }

      console.log("========== OFFLINE PAYMENT DEBUG END ==========");
      // ============ CASH/CHEQUE PAYMENT END ============
    }
  } catch (err) {
    console.error("Payment failed:", err);
