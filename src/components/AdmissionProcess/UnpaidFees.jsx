...
//         endpoint = "/student-fees/overdueFees";
...
//       const response = await axiosInstance.get(`${JAVA_BASE_URL}${endpoint}`, {
//   params,
// });
...
//     try {
//       setLoder(true);
//       const response = await axiosInstance.get("/student-fees/overdueFees");
...
const UnpaidFeesList = () => {
  ...
  const loadUnpaidFees = async () => {
    try {
      setLoading(true);
      let endpoint = "/student-fees/overdueFees";
      let params = {};
      ...
      if (
        ...
        // if (selectedClass) params.year_level_id = Number(selectedClass);
        if (selectedMonth) params.month = monthMap[selectedMonth];
      }
      else if (userRole === constants.roles.teacher) {
        if (yearLevelID) params.year_level_id = Number(yearLevelID);
        if (selectedMonth) params.month = monthMap[selectedMonth];
      }
      else if (userRole === constants.roles.guardian) {
        ...
        if (userID) params.student_id = Number(userID);
      }

      const response = await axiosInstance.get(
        `${JAVA_BASE_URL}${endpoint}`,
        { params }
      );
  ...
  const filteredFees = unpaidFees.filter((item) => {
    ...
    const matchesClass =
      !selectedClass ||
      item.class_name ===
      yearLevels.find((lvl) => lvl.id === Number(selectedClass))?.level_name;
  ...
  return (
    <div className="min-h-screen p-5 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 mb-24 md:mb-10">
      <div className="bg-white dark:bg-gray-800 max-w-7xl p-6 rounded-lg shadow-lg mx-auto">
        ...
        {/* Table */}
        <div className="overflow-x-auto rounded-lg max-h-[70vh]">
              ...
              {sortedFees.length === 0 ? (
                ...
                sortedFees.map((item, index) => (
                  <tr key={`${item.fee_id}-${index}`}>
                    ...
                    <td className="px-4 py-3">₹{item.original_amount}</td>
                    <td className="px-4 py-3">₹{item.paid_amount}</td>
                    <td className="px-4 py-3">₹{item.due_amount}</td>
                    <td
                      className={`inline-flex items-center px-3 py-1 rounded-md shadow-sm text-sm font-medium m-2 ${parseFloat(item.due_amount) === 0
                        ? "bg-green-100 text-green-600"
                        : "bg-red-100 text-red-600"
                        }`}
                    >
