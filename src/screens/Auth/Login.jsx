  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response?.accessToken) {
        ...
        // Store only IDs and roles (tokens already handled in AuthContext)
        localStorage.setItem("userRole", role);
        localStorage.setItem("userId", userId);
        if (studentId) localStorage.setItem("student_id", studentId);
        if (guardianId) localStorage.setItem("guardian_id", guardianId);
        if (teacherId) localStorage.setItem("teacher_id", teacherId);
        if (officeStaffId) localStorage.setItem("officeStaffId", officeStaffId);

        // Redirect based on role
        const normalizedRole = role.toLowerCase().replace(/[_\s]/g, "");
        let redirectPath = allRouterLink.login;
        switch (normalizedRole) {
          case "director": redirectPath = allRouterLink.directorDashboard; break;
          case "teacher": redirectPath = allRouterLink.teacherDashboard; break;
          case "guardian": redirectPath = allRouterLink.guardianDashboard; break;
          case "student": redirectPath = allRouterLink.studentDashboard; break;
          case "officestaff": redirectPath = allRouterLink.officeStaffDashboard; break;
        }
