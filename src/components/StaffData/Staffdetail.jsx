...
const Staffdetail = () => {
  ...
  const fetchJavaImage = useCallback(async (userId) => {
    ...
    try {
      // Step 1: Get image path from Java API
      const res = await axios.get(`${JAVA_BASE_URL}/users/getUserImage/${userId}`);
      ...
      // Step 3: Fetch actual image blob
      ...
      // Step 4: Cleanup old blob & create new
      cleanupBlobUrl(javaBlobUrlRef.current);
  ...
  // ============ FETCH JAVA IMAGE WHEN STAFF DATA CHANGES ============
  useEffect(() => {
    if (staffData?.user) {
      fetchJavaImage(staffData.user);
    }
  ...
  }, [staffData?.user, fetchJavaImage, cleanupBlobUrl]);

  // ============ IMAGE PRIORITY: Java > DRF ============
  let profilePicUrl = null;
  if (javaImageUrl) {
    profilePicUrl = javaImageUrl;
  } else if (staffData?.user_profile) {
    // DRF fallback - check if URL is already complete
    if (staffData.user_profile.startsWith("http")) {
      profilePicUrl = staffData.user_profile;
    } else {
      profilePicUrl = `${BASE_URL}${staffData.user_profile.startsWith("/") ? "" : "/"}${staffData.user_profile}`;
    }
  }
              ...
              <div className="mt-2">
                {profilePicUrl ? (
                  <img
                    ...
                    className="w-24 h-24 object-cover border rounded-full dark:border-gray-600"
                    onError={() => {
                      // If image fails to load, cleanup and reset
                      cleanupBlobUrl(javaBlobUrlRef.current);
                      javaBlobUrlRef.current = "";
                      setJavaImageUrl("");
                    }}
