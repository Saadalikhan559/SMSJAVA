...
export const Navbar = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      setProfileData({});
      setProfileImageError(false);
      setJavaImageUrl(""); // 🔥 Clear java image
      navigate("/login", { replace: true });
    } catch (error) {
      console.error("Logout error:", error);
    } finally {
  ...
  useEffect(() => {
    return () => {
      if (javaImageUrl && javaImageUrl.startsWith('blob:')) {
        URL.revokeObjectURL(javaImageUrl);
      }
    };
  }, [javaImageUrl]);

  /* ================= 🔥 PROFILE IMAGE LOGIC ================= */
  // Priority: Java Image > Django Image > Default
  let profilePicUrl = defaultProfileImage;

  if (isAuthenticated) {
    if (javaImageUrl) {
      // 🔥 First priority: Java image
