...
export const ViewDocuments = () => {
  ...
  // ✅ Function to view attachments (reused everywhere)
  ...
  details.forEach((doc) => {
    ...
    const name =
      doc.student_name || doc.guardian_name || doc.office_staff_name || doc.teacher_name;
    ...
    doc.document_types_read.forEach((dt) => {
      ...
      grouped[key].docs[type] = doc.files.map((file) =>
        file.file.replace("http://localhost:8000", `${BASE_URL}`)
      );
    });
  });
  ...
  const filteredData = Object.values(grouped).filter((person) => {
    ...
    const classMatch =
      selectedRole === "Student" ? selectedClass === "All" || person.yearLevel === selectedClass : true;
        ...
        <div className="w-full overflow-x-auto rounded-lg no-scrollbar max-h-[70vh]">
          <table className="min-w-full divide-y divide-gray-300 dark:divide-gray-700">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
              {[...filterBySearch].sort((a, b) => a.name.localeCompare(b.name)).map((person, idx) => (
                <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                  ...
                  {allDocTypes.map((type) => (
                    <td key={type} className="px-4 py-3 text-sm text-blue-600 dark:text-blue-400">
