...
const StudentDetails = () => {
  ...
  const fetchJavaStudentImage = useCallback(async (userId) => {
    if (!userId) return;
    ...
    try {
      ...
      javaBlobUrlRef.current = blobUrl;
      setJavaImageUrl(blobUrl);
    } catch (err) {
      console.error("Failed to fetch student Java image:", err);
      cleanupBlobUrl(javaBlobUrlRef.current);
      javaBlobUrlRef.current = "";
      setJavaImageUrl("");
    }
  ...
  useEffect(() => {
    if (student?.user) {
      fetchJavaStudentImage(student.user);
    }

    return () => {
      cleanupBlobUrl(javaBlobUrlRef.current);
    };
  }, [student?.user, fetchJavaStudentImage, cleanupBlobUrl]);

  // Computed Values
  let profilePicUrl = null;
  if (javaImageUrl) {
    profilePicUrl = javaImageUrl;
  } else if (student?.user_profile) {
    profilePicUrl = `${BASE_URL}${student.user_profile.startsWith("/") ? "" : "/"}${student.user_profile}`;
  }
